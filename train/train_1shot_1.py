...
def get_dataloader_few_shot():
    train_loader = torch.utils.data.DataLoader(MyDataset_pair(test_group=1),
        batch_size=args.b, shuffle=True, num_workers=0, pin_memory=True)

    val_loader = torch.utils.data.DataLoader(
        MyDataset_val(test_group=1, k_shot=1),
...
def main():
    ...
    Trainer(model=model,
            criterion=criterion,
            flag_val='just_val',  # 这个用来控制仅仅训练，或者每个epoch后val一次,'just_train','just_val','train_val'
