...
from dataset.seg.pascal_seg import MyDataset_pair
from dataset.seg.get_sbd_data_follow_SG import MyDataset_val
from models.model_1shot import Model
...
parser.add_argument('--experiment_dir', default='/media/yyw/JX_disk/yyw_disk/'
                                                'yyw/pycharm_pro/2019late/Local-Transformer/'
                                                'experiment/Model_1shot_0')
parser.add_argument('--model_dir', default='/media/yyw/JX_disk/yyw_disk/yyw/pycharm_pro/2019late/Local-Transformer/'
                                           'experiment/Model_1shot_0/model_save')
...
def crf(img, scores):
    '''
    the post-operator based on dense-CRF
    scores: prob numpy array with shape (C, H, W)
    img: image with shape (H, W)

    output:
     Q: the optimized segmentation results
     Q_score: its optimized results of probability map
    '''
...
def get_dataloader_few_shot():
    ...
    '''
    train_loader = torch.utils.data.DataLoader(MyDataset_pair(test_group=0),
        batch_size=args.b, shuffle=True, num_workers=0, pin_memory=True)

    val_loader = torch.utils.data.DataLoader(
        MyDataset_val(test_group=0, k_shot=1),
...
def get_model():
    '''
    To get the best model if it has been saved in dir.
    :return: model: The model with pre-trained parameters
    '''
    model = Model()
...
class Trainer():
    '''
    the class used to train and validate the model
    '''
    def __init__(self, model=None, criterion=None, flag_val=None, break_for_debug=True):
        '''
        :param model: the pre-trained model
        :param criterion: the cross-entropy loss
        :param flag_val: the set of {"just_val", "train_val"}
        :param break_for_debug: It is used to test the code
        '''
        self.model = model
    ...
    def get_R_truth(self, gt_que_1, gt_sup_1):
        '''
        To get the ground-truth of R
        :param gt_que_1: the ground-truth of query image
        :param gt_sup_1: the ground-truth of support image
        :return: The ground-truth of R
        '''
        gt_que_1 = gt_que_1.unsqueeze(1)
        gt_que_1 = F.interpolate(gt_que_1, (20, 20), mode='bilinear')
        gt_que_1 = gt_que_1.view(gt_que_1.size()[0], -1, 1)  # with size (HW, 1)

        gt_sup_1 = gt_sup_1.unsqueeze(1)
        gt_sup_1 = F.interpolate(gt_sup_1, (20, 20), mode='bilinear')
        gt_sup_1 = gt_sup_1.view(gt_sup_1.size()[0], 1, -1) # with size (1, HW)
    ...
    def train_pair(self):
        ...
        for batch_index, (data_list, gt_list, name_list, label_pair) in enumerate(self.train_loader_pair):
            ...
            ### save the segmentation results during training
            outputs = predict.data.cpu().numpy()
