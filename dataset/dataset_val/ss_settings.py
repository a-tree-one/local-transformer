import numpy as np
from dataset.dataset_val.util import Map
...
def get_cats(split, fold, num_folds=4):
    ...
    val_set = [fold*val_size+v for v in range(val_size)]  #验证数据集所包含的类别
    train_set = [x for x in range(num_cats) if x not in val_set] #训练数据集包含的类别
    if split=='train':
...
empty_profile = Map(
                ...
                coco_path = None,
                pascal_path = '',
                sbd_path = '/media/yyw/JX_disk/yyw_disk/datasets/sbd', ##the dir of sbd included three parts: 1)the dir of images: img 2) the dir of the ground truth: cls 3) the val.txt
                worker_num = 1)
...
fold0_train = Map(empty_profile,
    ...
    image_sets=['sbd_training',],
    pascal_cats = get_cats('train',0),
    # first_shape=[224,224],
    # second_shape=[224,224])
    # first_shape=[320,320],
    # second_shape=[320,320])
    # second_shape=None)
                  )
